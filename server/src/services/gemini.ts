...
const response = await gemini.models.generateContent({
    ...
    contents: [
        {
            text: "Transcreva o audio para portugues do Brasil. Seja preciso e natural na transcrição. Mantenha a pontuação adequanda e divida o texto em paragrafos quando for apropriado"
        },
        {
            inlineData: {
                mimeType,
...
export async function generateAnswer(question: string, transcription: string[]){
    ...
    - Se for citar o contexto, utilize o termo "conteudo";
    `.trim()
    const response = await gemini.models.generateContent({
        model,
        contents: [{
